...
export function ensureThoughtsRepoExists(
	...
	const isGitRepo = fs.existsSync(gitPath) && (fs.statSync(gitPath).isDirectory() || fs.statSync(gitPath).isFile())
 ...
 */
export function getRepoNameFromMapping(mapping: string | RepoMappingObject | undefined): string | undefined {
 ...
 */
export function getProfileNameFromMapping(mapping: string | RepoMappingObject | undefined): string | undefined {
	if (!mapping) return undefined
...
export function createThoughtsDirectoryStructure(config: ResolvedProfileConfig, repoName: string, user: string): void
